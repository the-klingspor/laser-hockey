...
def parseOptions():
    parser = argparse.ArgumentParser(
        description="ALRL2023 Competition Client.",
...
"""
    ...
    def step(
        ...
        if not isinstance(info["reward_closeness_to_puck"], Number):
            print(info["reward_closeness_to_puck"])
        self.num_steps += 1

        try:
            # Update UCBs and select next agent, if UCB ensemble
            if done or self.num_steps == MAX_STEPS:
                if hasattr(self.controller, "select_next_agent"):
                    winner = r / 10
                    self.controller.select_next_agent(winner)
            ...
            if self.validate_action(action):
                ...
                raise ValueError(f"Not valid action: {str(action)}")
        except:
    ...
    def game_done(
        self, ob: List[float], r: int, done: int, trunc: int, info: Dict, result: str
    ) -> None:
        if self.verbose:
            print(f"Winner: {info['winner']}")
            print(
...
def main(opts):
    controller = environment.hockey_env.BasicOpponent(weak=False)
    client = Client(
