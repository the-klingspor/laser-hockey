...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--experiment",
        type=int,
        help="Number of experiment (1-11).",
    )

    args = parser.parse_args()

    # Load all config dicts
    with open(f"configs/agent_config_e{args.experiment}.json") as f:
        agent_config = json.load(f)

    with open(f"configs/algorithm_config_e{args.experiment}.json") as f:
        algorithm_config = json.load(f)

    with open(f"configs/buffer_config_e{args.experiment}.json") as f:
        buffer_config = json.load(f)

    with open("configs/optimizer_config.json") as f:
